...
function initGlobalLoggerSync() {
	/* create the logger */
	if (!fs.existsSync(CONSTANTS.LOGDIR)) { fs.mkdirSync(CONSTANTS.LOGDIR); }
	let filewriter =
		require(`${CONSTANTS.LIBDIR}/FileWriter.js`).createFileWriter(CONSTANTS.LOGNAME, log_conf.closeTimeOut, "utf8");

	global.LOG = new Logger(CONSTANTS.LOGNAME, log_conf.max_log_mb * 1024 * 1024, filewriter);	// 100 MB log max
